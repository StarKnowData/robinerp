...
LRESULT XRightFrame::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	else if (wParam == IDT_SHOW_TALKBUF)
	{
		...
		{
			CStdString str = m_dqTalkBuff.front();
...
void XRightFrame::OnBnProp(BzDui::CButtonUI* pButton)
{
	...
	//体验场(免费场) 不能使用道具
	if (m_pXlogic->GetGameInfo()->dwRoomRule & GRR_EXPERCISE_ROOM)
	{
	...
	_stprintf(szKey,"%d",m_pXlogic->GetGameInfo()->uNameID);
...
void XRightFrame::Notify(TNotifyUI& msg)
{
	...
	else if (msg.sType == _T("itemselect"))
	{
		...
		if( msg.pSender->GetName() == _T("Talk_Cob_Inp")) 
		{
			...
			m_pXlogic->SendTalkMsg((TCHAR*)msg.pSender->GetText().GetData(),m_pXlogic->GetMeUserInfo()->dwUserID);
