...
void XLogic::QuitGame(bool bNotify,bool bNeedCheck)
{
    ...
    //    winApp->PostMessage(WM_GAME_QUIT,0,0);	// 短参不为0，表示不通知大厅
    //}
    ...
    case CM_USER_SEND_TIMES_MONEY:
        {
            ...
            //{
            //    wsprintf(str,"/:54获赠%s金币，已获赠%d次，已无获赠机会/:54",szNum,
            //        pMoneyChange->iSendTimes,pMoneyChange->iLastTimes);
            //}
            //else
            //{
            //    wsprintf(str,"/:54获赠%s金币，已获赠%d次，还有%d次获赠机会/:54",szNum,
            //        pMoneyChange->iSendTimes,pMoneyChange->iLastTimes);
            //}
...
int XLogic::InitPlayerListCtrl()
{
    ...
    /*if (pList->GetColNum()<12)
    {
		...
		str = fUserList.GetKeyVal("InforList","NickName","昵称");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(0,szStr,80,20);

		str = fUserList.GetKeyVal("InforList","Coin","乐豆");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(1,szStr,80,20);

		str = fUserList.GetKeyVal("InforList","WealthLevel","财富级别");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(2,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","Score","积分");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(3,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","ScoreLevel","积分级别");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(4,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","GameCount","总局数");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(5,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","WinGame","胜局数");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(6,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","LoseGame","输局数");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(7,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","Peace","和局数");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(8,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","WatchUser","旁观对象");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(9,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","Charm","魅力");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
        pList->InsertColumn(10,szStr,60,20);

		str = fUserList.GetKeyVal("InforList","CharmLevel","魅力级别");
		str.Replace(',', '\0');
		char2wchar(str.GetBuffer(str.GetLength()), szStr);
