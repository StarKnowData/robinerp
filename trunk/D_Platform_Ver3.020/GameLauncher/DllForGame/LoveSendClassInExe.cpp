...
BEGIN_MESSAGE_MAP(CLoveSendClassInExe, CLoadFileBmp)
	...
	ON_MESSAGE(WM_BUY_VIP,OnPannelBuyVIP)		// 从道具面板中传回购买VIP消息     add by wyl   11-5-21
	ON_MESSAGE(WM_USE_BOARDCAST,OnUseBoardcast)
	ON_MESSAGE(WM_BANK_CHECK_OUT,OnBankCheckOut)	// 从银行面板中取钱
	ON_MESSAGE(WM_GETFRIENDLIST,OnSendGetFriendList)		// 从道具面板中传回购买VIP消息     add by wyl   11-5-21
	ON_MESSAGE(WM_CHECK_MONEY,OnGameBankMoneyCheck) //游戏银行存取钱 jianguankun 2012.9.20
	ON_MESSAGE(WM_CHECK_MONEY2,OnGameBankMoneyCheck2) //游戏银行存取钱 jianguankun 2012.9.20
	ON_MESSAGE(WM_BANK2_CLOSE,OnGameBankClose) //关闭游戏银行 jianguankun 2012.9.20
...
bool CLoveSendClassInExe::InitFrame(GameInfoStructInExe * pGameInfo, CWnd *pGameRoom)
{
	...
	_tcscpy(m_szGameRoomName, pGameInfo->szGameRoomName); //modify lxl 2010-10-18 显示房间名字 
...
BOOL CLoveSendClassInExe::OnInitDialog()
{
	...
	m_pGameInfo->bShowUserInfo=(GetProfileInt(TEXT("ShowUserInfo"),FALSE)!=FALSE);  //PengJiLin, 2010-10-15, 缺省不显示玩家进出信息
...
void CLoveSendClassInExe::OnPaint()
{
	...
	//end of add by wlr 20100118 双缓存
