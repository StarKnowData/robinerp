...
// 		bool bSameIP=(pUserInfo->m_Rule.bSameIP==TRUE);
// 		if ((m_pDataManage->m_InitData.dwRoomRule&GRR_LIMIT_SAME_IP)!=0) bSameIP=true;
// 		if (bSameIP==false)
// 		{
// 			for (BYTE i=0;i<m_bMaxPeople;i++)
// 			{
// 				if ((m_pUserInfo[i]!=NULL)&&(m_pUserInfo[i]->m_Rule.bSameIP==TRUE))
// 				{
// 					bSameIP=true;
// 					break;
// 				}
// 			}
// 		}
// 		if (bSameIP==true||pUserInfo->m_Rule.bIPLimite)//不与IP相同或不于IP前几位的人游戏
// 		{
...
bool CGameDesk::CleanCutGameInfo()
{
	for (BYTE i=0;i<m_bMaxPeople;i++)
	{
		if ((m_pUserInfo[i]!=NULL)&&(m_bCutGame[i]==true))
		{
			...
			if (m_pDataManage->m_InitData.dwRoomRule & GRR_QUEUE_GAME)
			{
				m_pDataManage->AutoAllotDeskDeleteUser(pUserInfo); 
			}
			//UserLeftDesk(i,pUserInfo);
			MakeUserOffLine(i);
...
bool CGameDesk::ChangeUserPointContest(__int64 *arPoint, bool *bCut, int nTaxIndex)
{
	...
	OutputDebugString("DServer:CGameDesk::ChangeUserPointContest 64");
...
{
	OutputDebugString("--------平台-服务端：用户点击同意开始按钮！------");
	...
	{		
		OutputDebugString("--------平台-服务端：用户点击同意开始但身上金币不足！------");
	...
	//{
	//	CString str;
	//	str.Format("UserAgreeGame :%s,%d",m_pUserInfo[bDeskStation]->m_UserData.nickName,
	//		m_pUserInfo[bDeskStation]->m_UserData.bUserState);
	//	::OutputDebugString(str);
	//}

    //PengJiLin, 2010-5-19, 排队机场人数不够时须重新参与排队
...
bool CGameDesk::GameFinishContest(BYTE bDeskStation, BYTE bCloseFlag)
{
	...
	for (BYTE i=0;i<m_bMaxPeople;i++)
	{
		...
		// 如果玩家已经没钱了，则马上离开桌子
		CString str;
		str.Format("DServer:CGameDesk::GameFinishContest User[%d] = %s  ContestScore = %I64d LowChip = %I64d _bfinish = %d",
			i,m_pUserInfo[i]->m_UserData.szName,
			m_pUserInfo[i]->m_UserData.i64ContestScore,
			(int)m_pDataManage->m_InitData.i64LowChip,
			_bfinish);
		OutputDebugString(str);

		if ((m_pUserInfo[i]->m_UserData.i64ContestScore < (int)m_pDataManage->m_InitData.i64LowChip)
			&&(m_pDataManage->m_InitData.dwRoomRule & GRR_CONTEST) || _bfinish)
		{
			//m_pDataManage->m_TCPSocket.SendData(pUserInfo->m_uSocketIndex, MDM_GR_USER_ACTION, ASS_GR_NoMoneyTickRoom, 0, 0);
			if (_bfinish) m_pDataManage->m_TCPSocket.SendData(m_pUserInfo[i]->m_uSocketIndex, MDM_GR_USER_ACTION, ASS_GR_CONTEST_GAMEOVER, 0, 0);
			else m_pDataManage->m_TCPSocket.SendData(m_pUserInfo[i]->m_uSocketIndex, MDM_GR_USER_ACTION, ASS_GR_CONTEST_KICK, 0, 0);
			MakeUserOffLine(i);
		}
