...
bool CGameMainManage::OnUserActionMessage(NetMessageHead * pNetHead, void * pData, UINT uSize, ULONG uAccessIP, UINT uIndex, DWORD dwHandleID)
{
	...
	case ASS_GR_JOIN_QUEUE:	///<有用户加入排队机
		...
		//	if (pUserInfo->m_UserData.bUserState == USER_LOOK_STATE)//没有坐到桌子
			{
...
bool CGameMainManage::OnDataBaseResult(DataBaseResultLine * pResultData)
{
    ...
    case DTK_GR_SEND_MONEY_ON_TIMES_COUNTS:     // PengJiLin, 2011-4-15, 在线时长、局数送金币
        {
...
bool CGameMainManage::SendRoomInfo(CGameUserInfo * pUserInfo, UINT uSocketIndex, DWORD dwHandleID,DL_GR_O_LogonResult * pLogonResult)
{
	...
	do
	{
		...
		{
			////wushuqun 2009.5.22
			//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成");
			//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成-开始发送发送在线用户信息");
			...
			{
				////wushuqun 2009.5.22
				//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成-发送发送在线用户信息失败");
				return false;
			}
			////wushuqun 2009.5.22
			//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成-发送发送在线用户信息成功");

		}
		else 
		{
			////wushuqun 2009.5.22
			//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成-正在发送发送在线用户信息");
			if (m_TCPSocket.SendData(uSocketIndex,bBuffer,(UINT)uCopyPos*sizeof(UserInfoStruct),
				MDM_GR_USER_LIST,ASS_GR_ONLINE_USER,ERR_GR_LIST_PART,dwHandleID)!=uCopyPos*sizeof(UserInfoStruct))
			{
			//	//wushuqun 2009.5.22
			//::OutputDebugString("函数 CGameUserInfoManage::FillOnLineUserInfo: 获取在线用户-完成-正在发送发送在线用户信息-失败");
				return false;
			}
			//wushuqun 2009.5.22
			::Sleep(10);
...
void CGameMainManage::NewQueueGame()
{
	...
	{
		//ErrorPrintf("撮桌机制不应返回空指针--[%d]", __LINE__);
		return;
	}
	/// 目标桌号
	int nDeskNo=0;
