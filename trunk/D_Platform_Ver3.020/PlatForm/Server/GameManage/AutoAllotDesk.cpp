...
void CAutoAllotDesk::AllotDesk(int& iDeskCount, int* &arrDesk)
{
	int _usercount = 0;
	...
	//if (_usercount >= m_iCanQueuePeople || m_UserList.size() <= m_iCanQueuePeople)
	{
		for (int _iBegin = 0; _iBegin < (int)m_UserList.size() - m_iMinDeskPeopleCount + 1; _iBegin++)
		{
			if (CanSit(m_UserList[_iBegin]))
			{
				int _iCount = 0;
				for (int i = _iBegin; i < (int)m_UserList.size() ; i++)
				{
					if (CanSit(m_UserList[i]) && CanSit(_iDeskIndex, _iCount, i) && CanRobotAutoSit(_iDeskIndex, _iCount, i))
					{
						...
						//ContestPrintf("AutoAllot = %d", _unsitcount);
						...
						{
							//ContestPrintf("AutoAllotDesk::AllotDesk=> 搓桌失败, 当前已有桌数(%d), 人数不够, i(%d), iCount(%d), m_iDeskPeopleCount(%d), m_iMinDeskPeopleCount(%d)",
							//	_iDeskIndex, i, _iCount, m_iDeskPeopleCount, m_iMinDeskPeopleCount);
						}
					}
					else
					{
						//ContestPrintf("AutoAllotDesk::AllotDesk=> 搓桌失败: i(%d) UserID(%d), NickName(%s), IP(%s), Weight(%d), CanSit1(%d), CanSit(%d), CanRobotAutoSit(%d)",
						//	i, m_UserList[i].m_pGameUserInfo->m_UserData.dwUserID, m_UserList[i].m_pGameUserInfo->m_UserData.nickName, ConvertIP2(m_UserList[i].m_pGameUserInfo->m_UserData.dwUserIP), m_UserList[i].m_iWeight,
						//	CanSit(m_UserList[i]), CanSit(_iDeskIndex, _iCount, i), CanRobotAutoSit(_iDeskIndex, _iCount, i));
					}
				}
			}
		}
...
bool CAutoAllotDesk::CanSit(const QueueInfo_t& user)
{
