...
void CCenterServerManage::GetURL(char *strKey)
{
	...
	if (NULL == strKey)
	{
		strValue = f.GetKeyVal(_T("GateServer"),_T("RoomRollWords"),_T("欢迎来到泊众棋牌游游戏中心！"));///< 客户端滚动条广告地址
...
bool CCenterServerManage::CMainserverList::RandAServer(char* ipaddr,long& port)
{
	...
	{
		//////////////////////////////////////
		///////////Kylin 20090114 Z服务负载均衡，平均分配法
		static int curi=0;
		int count=m_List.GetCount ();//服务器IP列表的总数量
		long t=GetCurrentTime() + rand();//随机选择一个书
		//int curi=(t) % count ;
		...
		curi++;
		if(curi>=m_List.GetCount()) curi=0;
		//////////////////////////////////////


		//int count=m_List.GetCount ();//服务器IP列表的总数量
		//long t=GetCurrentTime() + rand();//随机选择一个书
		//int curi=(t) % count ;int i=0;//余数，发现取余数比乘百分百，然后取整数简单
...
bool CCenterServerModule::InitService(ManageInfoStruct * pInitData)
{
	try
	{
		return m_LogonManage.Init(pInitData,NULL);//&m_DataBaseHandle);
	}
