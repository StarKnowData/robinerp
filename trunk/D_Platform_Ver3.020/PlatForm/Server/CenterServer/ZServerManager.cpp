...
{
	//if (pNetHead->uMainID == 1 && pNetHead->uAssistantID == 1) return true;
	//if (pNetHead->bMainID== MDM_CONNECT)		//连接信息
	//{
	...
	if (pNetHead->bMainID==MDM_ZD_PACKAGE)
	{
		...
		{
			sockaddr_in socket;
			//m_TCPSocket.GetServerInfo(uIndex, &socket);
			//char szIP[20] = "";
			//lstrcpy(szIP,inet_ntoa(socket.sin_addr));
