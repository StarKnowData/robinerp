...
//提炼出来的进入游戏房间 add by lxl 2010-10-08
bool CMainRoomEx::EnterRoom(CAFCRoomItem * pGameRoomItem)
{
		...
		&&(pComRoomInfo->uComType!=TY_MONEY_GAME)))
	{
		...
		return true;		
		/*if (AFCMessageBox(TEXT("您的版本过旧，不能进入此游戏大厅，是否现在下载？"),MESSAGE_TITLE,MB_ICONQUESTION|MB_YESNO,this)==IDYES)
		{
		//下载文件
		CString strBuffer;
	...
	for (BYTE i=0;i<sizeof(m_RoomInfo)/sizeof(m_RoomInfo[0]);i++)
	{
		...
		if(pGameRoomItem->m_RoomInfo.uRoomID==m_RoomInfo[i].pComRoomInfo.uRoomID)
		{
			if (m_RoomInfo[i].pComRoomInfo.uComType != TY_MATCH_GAME ||
                (m_RoomInfo[i].pComRoomInfo.dwRoomRule & GRR_QUEUE_GAME))   // PengJiLin, 2010-9-19, 比赛排队机场
			{
...
// 	{
// 		str = fMsg.GetKeyVal("MainRoom","CoinRoomAllowOne","游戏币场只能进一个!");
// 		AFCMessageBox(str,strTitle);//,MB_ICONQUESTION,this);
// 		return false;
// 	
// 	}
...
/*				else
	...
	{			
		...
		if(DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION,MESSAGE_TITLE,true,strMessage)==IDYES)
		{
			...
			/// CloseSocket (true); ///< 更新时不需要断开与Z服务器的连接，zxd注释于20100315
			return true;
		}
		else
		{
...
//		s.Format("无法得知当前游戏版本,重新安装此游戏可能会修复此问题\n是否现在下载?");
		//if(AFCMessageBox(str,szGameName,AFC_YESNO)==IDOK)
		if(DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION,szGameName,true,str)==IDYES)
	...
	//			s.Format("正在更新的游戏太多,请后再试.");
		...
		{//超时 update.exe 还没有连接
			...
			{
				::TerminateProcess( m_CSocket32.GetProcessInfo().hProcess, 0 );//结束进程 update.exe
			}
			m_CSocket32.Close();

			CString str;
			CString strTitle;
			str = fMsg.GetKeyVal("MainRoom","DownLoadOver","下载失败");
	...
	for (BYTE i = 0;i < MAX_GAME_ROOM; ++i)
	{
		...
		if(NULL != pGameRoom)
		{
			...
			if (pGameRoom->m_GameInfo.uNameID== pGameRoomItem->m_RoomInfo.uNameID)
			{
                ...
                DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,strTip);
				return 0;
			}
		}
	}
...
// 		}
	...
	if(pGameRoomItem->m_RoomInfo.iLeaseID != 0)
	{
		...
		{
			//AFCMessageBox("房间租赁已经结束");
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"房间租赁已经结束");
			return false;
		}
	}
	...
	try
	{
        ...
        // PengJiLin, 2010-9-13, 踢人卡、防踢卡使用时间设置功能
        ...
        // PengJiLin, 2010-9-20, 如果是比赛场，并且没有排队机，隐藏右侧控件
	...
	if (pRoomInfoItem->pComRoomInfo.uComType != TY_MATCH_GAME ||
        (pRoomInfoItem->pComRoomInfo.dwRoomRule & GRR_QUEUE_GAME))   // PengJiLin, 2010-9-19, 比赛排队机场
	{
...
{
	//如果是返回进入密码房间成功的消息，则进入房间，否则按原有方式处理 add by lxl 2010-10-08
	if (ASS_GP_ROOM_PASSWORD == pNetHead->bAssistantID)
	{
		...
		{
			//AFCMessageBox("您的密码输入错误，暂时不能进入该房间游戏!");
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"您的密码输入错误，暂时不能进入该房间游戏!");
	...
	case MDM_GP_REQURE_GAME_PARA:	//中心服务全局参数//请求游戏全局参数
		{
			...
			//	{
			//		TCHAR sz[200];
			//		wsprintf(sz,TEXT("升级程序启动失败,您可能需要重新安装此程序."));
			//		AFCMessageBox(sz,TEXT("升级失败"));
			//	}
			//	CloseSocket (true);
			//	AfxGetApp( )->m_pMainWnd->DestroyWindow();
			//	return true;		
			...
			if(strcmp(s.GetBuffer(),strVersion)!=0)
			{
				...
				//	strTitle = fMsg.GetKeyVal("MainRoom","UpdateError","升级失败");
				//	str = fMsg.GetKeyVal("MainRoom","UpdateErrorReinstall",
				//		"升级程序启动失败. 重新安装大厅可能会修复此问题.");
	...
	case MDM_GP_USERINFO: //用户资料管理 JianGuankun 2011.11.13
		{
			if (ASS_GP_USERINFO_ACCEPT == pNetHead->bAssistantID)
			{
                ...
                DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"修改成功","系统提示信息");
			...
			else if (ASS_GP_USERINFO_NOTACCEPT == pNetHead->bAssistantID)
			{
                ...
                DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"修改失败","系统提示信息");
			}
			...
			if(ASS_GR_TRANSFER_MONEY == pNetHead->bAssistantID)		//转账成功后，提示消息给被转账人 add by xqm,lxl 2010-11-10  
			{
				...
				{					
					if (pTransfer->destUserID==m_PlaceUserInfo.dwUserID)
					{
						...
						//在大厅或房间弹出提示消息
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",true,strBuffer);
					}		
				}
			}
			return true;
	...
	case MDM_GP_BIND_MOBILE:
		{
			...
			if( pBind->dwCommanType == 0 )	//请求解除绑定手机结果
			{
				...
				if (!m_pSafeCenterWnd)
				{
					DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,"已解除手机绑定。");
				}
			}
			else if( pBind->dwCommanType == 1)//请求绑定手机结果
			{
				...
				{
					DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,"绑定手机成功！");
				}
				//end add
			}
			...
			else if (pNetHead->bAssistantID == ASS_EXCHANGE)
			{
				...
				if (_p != NULL)
				{
					...
					if (pNetHead->bHandleCode == 1)
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONERROR,"系统提示信息",false,"兑换出错");
					if (pNetHead->bHandleCode == 2)
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONERROR,"系统提示信息",false,"兑换出错");
					if (pNetHead->bHandleCode == 3)
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,"魅力值点数不够!");
					if (pNetHead->bHandleCode == 4)
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,"魅力值点数不够!");
					if (pNetHead->bHandleCode == 5)
						DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,"兑换成功!");
				}
			}
			...
			if (pNetHead->bAssistantID == ASS_GP_SMS_VCODE) //收到短信验证码
			{
				...
				if (_p != NULL)
				{
					if (::strlen(_p->szVCode) == 0)
					{
						...
						DUIOkeyMsgBox(m_hWnd,false,"未能正确获取手机验证码，请重试。");
...
void CMainRoomEx::OnLockAccountResult( NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	//	strErrorMsg = TEXT("您输入的密码有误，请查证后再次尝试!");
		...
		DUIMessageBox(m_hWnd,MB_OK|MB_ICONERROR,"系统提示信息",false,strErrorMsg);
		return;
	}
...
//			BZSoundPlay(this, "music/关锁.mp3", 0);
	...
	DUIMessageBox(m_hWnd,MB_OK|MB_ICONINFORMATION,"系统提示信息",false,strErrorMsg);
...
bool CMainRoomEx::OnSocketCloseEvent()
{
	...
	if(m_bConnectingToCenterServer)// && m_bGetPara)
	{
		...
		{
			//AFCMessageBox("非法数据包！");
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"非法数据包！");
			AfxGetApp( )->m_pMainWnd->DestroyWindow ();
			return true;
		}
		m_bConnectingToCenterServer = false;
...
bool CMainRoomEx::OnSocketConnectEvent(UINT uErrorCode, CTCPClientSocket * pClientSocket)
{
	...
	if (uErrorCode!=0)	//连接出现错误
	{
		...
		//AFCMessageBox(m_TCPSocket->TranslateError(uErrorCode),MESSAGE_TITLE);//,MB_ICONQUESTION,this);
        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,m_TCPSocket->TranslateError(uErrorCode));
...
// 	//闪屏第3：将DoMoal()==OK后的操作延时100ms操作

// 	if (Logon.DoModal()==IDOK)////////////////点登陆执行的代码
// 		SetTimer(1,100,NULL);
	...
	else
	{
		...
		else
		{
				...
				Glb().m_CenterServerPara.m_iMainserverPort))
			{
                ...
                DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"连接出现错误，请确认是否配置了网络协议！");
...
BOOL CMainRoomEx::PreTranslateMessage(MSG * pMsg)
{
	...
	if (pMsg->message==WM_KEYDOWN)		
	{
		...
		{
			CString CheckVersionHotKey = "游戏平台测试版 2010.08.25\n正易龙科技版权所有！";
...
bool CMainRoomEx::OnLogonMessage(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	...
	else if (pNetHead->bAssistantID == ASS_GP_LOGON_LOCK_VALID) //机器被锁定了 added by JianGuankun 2012.5.16
	{
		...
		DUIOkeyMsgBox(m_hWnd,true,"您好，本帐号已经在其他机器上锁机！<n>请使用正确的机器来登录。");
	...
	else if (pNetHead->bAssistantID==ASS_GP_LOGON_SUCCESS)	//登陆成功
	{
		...
		m_TopWnd->SetLockType(m_PlaceUserInfo.iLockMathine);	//锁定机器 zxj 2009-11-12
		m_TopWnd->FixControlStation();							//锁定机器 zxj 2009-11-12
...
//		((CGamePlaceDlg *)AfxGetMainWnd())->SetWindowTitle(m_strTitle);
		...
		//{
		//	TCHAR szMsg[MAX_PATH];
		//	wsprintf(szMsg,"上次登陆时长所送的金币数[%d],已到您银行帐上,请查收!",m_PlaceUserInfo.dwTimeIsMoney);
		//	MessageBox(szMsg);
		//}
        ...
        // PengJiLin, 2010-6-2, 处理第三方登录, 发送登录成功消息。(鸥朴)
	...
	case ASS_GP_BANK_OPEN:	///< 打开银行
		{
			...
			else if (pNetHead->bHandleCode == HC_GP_BANK_OPEN_SUC)
			{
				...
				if(pBank->nVer == 1) //旧版银行
				{
					...
					/*DUIOkeyMsgBox(m_hWnd,false,"请进入房间再使用银行");*/
				}
...
bool CMainRoomEx::StartDownLoad(TCHAR filename[],CAFCRoomItem * pGameRoomItem)
{
	...
	//			s.Format("正在更新的游戏太多,请后再试.");
		...
		//	  
		//	DWORD dwWrite=0;
		//	//m_AnonymousPipe.Write( &updateMsg ,sizeof(UpdateStructMsg),dwWrite);//发送到子进程
		//	m_CSocket32.Send( &updateMsg ,sizeof(UpdateStructMsg));//发送到子进程

		//	//在更新列表  m_UpdateList 增加节点
		//	UPDATE *lp=(LPUPDATE)malloc(sizeof(UPDATE));
		//	lp->hProcess=ProcessInfo.hProcess;
		//	lp->uNameID=pGameRoomItem->m_RoomInfo.uNameID;
		//	lp->pGameRoomItem=pGameRoomItem;
...
LRESULT CMainRoomEx::OnOpenBank(WPARAM wParam, LPARAM lParam)
{
	for(int i = 0; i < MAX_GAME_ROOM; ++i)
	{
		if(m_RoomInfo[i].pGameRoomWnd != NULL)
		{
            ...
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请到游戏房间中使用银行。");
			return 0;
		}
	}
