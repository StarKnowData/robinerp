...
void CBankVerifyWnd::OnOK()
{
	if (m_pPwdEdt->GetText().GetLength() == 0)
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请填写银行密码");
		m_pPwdEdt->SetFocus();
		return;
	}
	...
	if (m_pVCodeEdt->GetText().GetLength() == 0)
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请填写验证码");
	...
	if (!m_pVCCreater->IsVerify(m_pVCodeEdt->GetText()))
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"验证码不正确");
		m_pVCodeEdt->SetFocus();
		return;
	}
	...
	else
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"银行密码不正确，请再输入");
		m_pPwdEdt->SetFocus();
	}
