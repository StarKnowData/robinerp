...
void CFindUserDlg::OnOK()
{
	...
	if ((m_UserName.GetWindowTextLength()==0))//&&(m_UserAcc.GetWindowTextLength()==0))
	{
	    ...
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("FindUserDlg","InputUserName","您没有输入要查找的用户昵称"));
		m_UserName.SetFocus();
		return;
	}
	if (m_pParentWnd!=NULL)
	{
		...
		else 
		{
	        ...
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("FindUserDlg","NoUserInfor","没有找到您想查找的用户信息！"));
			m_UserName.SetFocus();
		}
	}
	return;
