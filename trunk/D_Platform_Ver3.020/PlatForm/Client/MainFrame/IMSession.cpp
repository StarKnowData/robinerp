...
void CIMSession::OnBnClickedSend()
{
	try
	{
		...
		if(!m_bOnline)
		{
	        ...
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("IMDlg","Tip","提示"),false,fMsg.GetKeyVal("IMDlg","NotSendToOffLineUser","对不起，您不能给不在线用户发消息！"));
			return;
		}
		UpdateData();
		m_strSend.TrimLeft();
		...
		if(m_strSend.GetLength()==0)
		{
	        ...
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("IMDlg","Tip","提示"),false,fMsg.GetKeyVal("IMDlg","NotSendNullMsg","对不起，您不能发送空的消息"));
			return;
		}
