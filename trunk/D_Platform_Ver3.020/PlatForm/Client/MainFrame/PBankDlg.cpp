...
void CPBankDlg::OnBnClickedAccountOK()
{
	...
	if (IDYES != DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION,"银行",true,szMsg))
	{
		BZSoundPlay(this, "music/错误提示.mp3", 0);
...
void CPBankDlg::OnBnClickedTransAll()
{
    ...
    if (strPSW.GetLength()<1)
    {
		...
		//AFCMessageBox("请输入密码！");
        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请输入密码！");
        return;
    }
    ...
    if (0 == atol(strID))
    {
        ...
        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请输入正确的玩家ID！");
        return;
    }
    ...
    if (IDYES != DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION,"银行",true,strMsg))
    {
        return;
    }
