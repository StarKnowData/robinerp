...
void CNormalBitmapButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (m_bIsMouseMove)
	{
		...
		///{{ Added by zxd 20100709 删除创建的GDI对象
		Pen.DeleteObject();
...
{
	//播放声音
	if (CGameImageLink::m_ButtonResource.m_bSound==true && m_bIsPlayMusic == true)
...
//构造函数
CNormalBitmapButtonNew::CNormalBitmapButtonNew():CNormalBitmapButtonEx()
{
	m_pTipWnd = NULL;

	m_dwTipDelay = 0;

	::memset(m_szTipText,NULL,sizeof(m_szTipText));
}

//析构函数
CNormalBitmapButtonNew::~CNormalBitmapButtonNew()
{
	if (m_pTipWnd)
	{
		m_pTipWnd->Close();
...
BEGIN_MESSAGE_MAP(CNormalBitmapButtonNew, CNormalBitmapButtonEx)
	ON_WM_TIMER()
	ON_WM_MOUSEMOVE()
	ON_MESSAGE(WM_MOUSELEAVE,OnMouseLeave)
END_MESSAGE_MAP()
...
//鼠标移动消息
void CNormalBitmapButtonNew::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bIsMouseMove)
	{
		//鼠标移入按钮，显示标签
		if (!m_pTipWnd)
		{
			if (::strlen(m_szTipText) > 0)
			{
				...
				{
					SetTimer(102,m_dwTipDelay,NULL);
				}
			}	
		}

		GetParent()->PostMessage(AFC_BTN_MOUSE_IN,(WPARAM)m_hWnd,NULL);
	}

	CTrueBitmapButton::OnMouseMove(nFlags,point);

	return;
}

//鼠标离开按钮消息
LRESULT CNormalBitmapButtonNew::OnMouseLeave(WPARAM wparam, LPARAM lparam)
{
	KillTimer(102);
	if (m_pTipWnd)
	{
	...
	GetParent()->PostMessage(AFC_BTN_MOUSE_OUT,(WPARAM)m_hWnd,NULL);

	return CTrueBitmapButton::OnMouseLeave(wparam,lparam);
}

void CNormalBitmapButtonNew::SetFlicker(bool bFlicker)
{
	m_bflicker = bFlicker;
	if (bFlicker)
...
//设置标签弹出的时间延迟
void CNormalBitmapButtonNew::SetTipDelay(DWORD dwMillSecond)
{
	m_dwTipDelay = dwMillSecond;
}

void CNormalBitmapButtonNew::OnTimer(UINT nIDEvent)
{
	if(nIDEvent==100)
	{
		KillTimer(100);
	...
	if (nIDEvent == 102)
	{
		...
		int iX = rc.left - 70;
		int iY = rc.top - 31;
		::GetWindowRect(m_pTipWnd->GetHWND(),&rc);
		::MoveWindow(m_pTipWnd->GetHWND(),iX,iY,rc.right - rc.left,rc.bottom - rc.top,true);
		m_pTipWnd->SetLabText(m_szTipText);
		m_pTipWnd->ShowWindow(SW_SHOW);
	}
}

bool CNormalBitmapButtonNew::SetTipText(char* szTipText)
{
	strcpy(m_szTipText,szTipText);
	return true;
}	