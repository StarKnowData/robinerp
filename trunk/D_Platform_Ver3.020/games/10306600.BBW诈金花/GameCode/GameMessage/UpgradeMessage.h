...
struct GameStation_2 : GameStationBase
{
	...
	//   房间相关信息
	__int64					iLimitNote;						//下注封顶值   明注 / 暗注
	int					iBaseNote;						//底注	       暗注
	int                 iGuoDi;                         //锅底
	__int64                 iLimtePerNote;                   //            暗注上限，明注需要 * 2
...
struct GameStation_4 : GameStationBase
{
	...
	__int64				iTotalNote[PLAY_COUNT];			//用户当前下注
	__int64             iThisGameNote[PLAY_COUNT];		//本局游戏下注数(明注)
