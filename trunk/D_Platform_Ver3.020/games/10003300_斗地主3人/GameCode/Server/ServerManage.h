...
#define TIME_JIAO_FEN               38              //叫分定时器
#define TIME_SHOW_CARD              39              //亮牌
#define TIME_START_GAME				40				//比赛开始时间设置
#define TIME_CHUI					41				//锤
//游戏结束标志定义
...
protected:
	...
	int                     m_iHistroyData[20] ;                     //出牌历史信息
	BYTE					m_byUserChui[PLAY_COUNT];				//记录玩家是否加锤 1:未知状态,0:不锤,2:锤
...
private:
	...
	static	int m_iHammerMoney;		//加锤时扣的钱
...
public:
	...
	///检测用户是否为Vip用户
	BOOL  UserVipVertify(BYTE bDeskStation) ; 
	//填充日志表
	...
	BYTE GetNoOutCard();
    ///是否是vip用户
	BOOL IsVipUser(BYTE bDeskStation) ; 
	///处理锤动画
	void HandleChuiAction(BYTE bDeskStation, bool bType);
////道俱
