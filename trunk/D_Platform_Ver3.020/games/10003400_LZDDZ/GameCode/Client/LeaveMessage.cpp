...
BOOL CLeaveMessage::OnInitDialog()
{
	...
	TCHAR path[MAX_PATH];
	wsprintf(path,TEXT(".\\image\\%s\\thingmsg.bmp"),GET_SKIN_FOLDER(szTempStr));
	m_bk.SetLoadInfo(path,true);
	SetWindowPos(&wndBottom,0,0,m_bk.GetWidth(),m_bk.GetHeight(),SWP_NOMOVE|SWP_NOZORDER);
	CenterWindow(GetParent());

//	TCHAR path[MAX_PATH];
	wsprintf(path,TEXT(".\\image\\%s\\agree_bt.bmp"),GET_SKIN_FOLDER(szTempStr));
	m_btOk.LoadButtonBitmap(path,false);
	wsprintf(path,TEXT(".\\image\\%s\\oppose.bmp"),GET_SKIN_FOLDER(szTempStr));
