...
BOOL CGameSet::OnInitDialog()
{
	...
	TCHAR path[MAX_PATH];
	wsprintf(path,TEXT(".\\image\\%s\\ok_bt.bmp"),GET_SKIN_FOLDER(szTempStr));
	m_Ok.LoadButtonBitmap(path,false);
	wsprintf(path,TEXT(".\\image\\%s\\cancel_bt.bmp"),GET_SKIN_FOLDER(szTempStr));
	m_Cancel.LoadButtonBitmap(path,false);
	wsprintf(path,TEXT(".\\image\\%s\\game_set.bmp"),GET_SKIN_FOLDER(szTempStr));
