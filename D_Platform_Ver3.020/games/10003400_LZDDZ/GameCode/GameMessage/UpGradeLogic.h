...
public://[辅助函数]
	...
	void CompareCard(BYTE bCard1[], int iCardCount1,BYTE bCard2[], int iCardCount2,	BYTE bSameCard[], int  &iSameCount,BYTE bDifferCard[],int  &iDifferCount);
	////////////////////////////////////////////////////////////////////////////////////////////////////
	//[牌型相关]
public:
    //获取牌型
	...
	//4+软张牌 炸弹
	BOOL IsShamBomb(BYTE iCardList[], int iCardCount, bool bExtVal=false);
	//同花炸弹
...
public:
	...
	BOOL CanOutCard(BYTE iOutCard[], int iOutCount, BYTE iBaseCard[], int iBaseCount, BYTE iHandCard[], int iHandCount,bool bFirstOut = false);
		//是否可以出牌
	BOOL ReplaceCanOutCard(BYTE iOutCard[], int iOutCount, BYTE iBaseCard[], int iBaseCount, BYTE iHandCard[], int iHandCount,ReplaceStruct & ReplaceCardList, bool bFirstOut = false);
...
public:	
	...
	//提取软炸弹(张数默认为4)
	BOOL TackOutShamBomb(BYTE iCardList[],int iCardCount,BYTE iResultCard[],int &iResultCardCount,int iNumCount=4);
	//提取王炸
...
public:
	...
	//得到代替牌型
	int GetCardReplaceShape(BYTE iCardList[],int iCardCount, ReplaceStruct & buf);
    ///获取手上最大的牌值
	int GetMaxCard(BYTE iBaseCard[] , int iBaseCardCount) ; 
};