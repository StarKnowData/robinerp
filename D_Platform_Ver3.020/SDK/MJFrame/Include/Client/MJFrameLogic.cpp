	...
	if (pMJCtrl != NULL)
	{
		...
		for (BYTE i = 0; i < mjSet.CurrentLength(); i++)
		{
			...
			case ACTION_QUADRUPLET_CONCEALED:  // 暗杠
				...
				break;
			case ACTION_QUADRUPLET_DNXB:       // 东南西北
			case ACTION_QUADRUPLET_ZFB:        // 中发白X
				{
	...
	if (pCmd->ucFlag & 0x04)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			if (pCmd->ucTile[2][i * 4] != 0)
			{
				...
				{
					tileSet.m_ucFlag = ACTION_QUADRUPLET_DNXB;      // 东南西北杠
				}
				...
				{
					tileSet.m_ucFlag = ACTION_QUADRUPLET_ZFB;       // 中发白X杠
				}
				else if (m_TilesHand[m_byMyStation].GetTileCount(tileSet.m_ucTile[0]) == 4)
				{
					tileSet.m_ucFlag = ACTION_QUADRUPLET_CONCEALED; // 暗杠
				}
				else if (m_TilesHand[m_byMyStation].GetTileCount(tileSet.m_ucTile[0]) == 3)
				{
					tileSet.m_ucFlag = ACTION_QUADRUPLET_REVEALED;  // 明杠
				}
				else
				{
					tileSet.m_ucFlag = ACTION_QUADRUPLET_PATCH;     // 补杠
				}
	...
	case CTN_Block_BTN_Gang:  // 杠
		{
			...
			else
			{
				...
				else
				{
					...
					if (mjHand.IsHave(TILE_EAST) && mjHand.IsHave(TILE_SOUTH) && mjHand.IsHave(TILE_WEST) && mjHand.IsHave(TILE_NORTH))
					{
						cmd.ucFlag = ACTION_QUADRUPLET_DNXB;      // 东南西北杠
					}
					else if (mjHand.IsHave(TILE_ZHONG) && mjHand.IsHave(TILE_FA) && mjHand.IsHave(TILE_BAI)
					     && (mjHand.IsHave(TILE_ZHONG, 2) || mjHand.IsHave(TILE_FA, 2) || mjHand.IsHave(TILE_BAI, 2)))
					{
						cmd.ucFlag = ACTION_QUADRUPLET_ZFB;       // 中发白X杠
					}
					else
					{
...
void CMJFrameLogic::OnRequestBlockSelect(UINT32 uControlID)
{
	...
	if (pMJCtrl != NULL)
	{
		...
		{
			if (tileInfo[i].bSelect && (tileInfo[i].dwReserve1 & 0x00000001) != 0x00000001 /*不可选*/ ) 
			{
				nIndex = i;
				break;
			}
			 ...
			 || tileSet.m_ucFlag == ACTION_QUADRUPLET_ZFB)
			{
				...
				if (mjHand.IsHave(tile))
				{
					...
					if (tile >= TILE_EAST && tile <= TILE_BAI)
					{
						...
						{
							// 拥有4张相同牌的杠判断
							// 例如: 中中发发白白白白, 会导致中发白白这种杠组合不提示, 会被白白白白杠优先显示, 所以把4张牌分开显示, 前面二张提示中发白白, 后面二张提示白白白白
							int nFirstPos = 0;
							for (j = 0; j < m_TilesHand[m_byMyStation].CurrentLength(); j++)
							{
								if (m_TilesHand[m_byMyStation].GetTile(j) == tile)
								{
									nFirstPos = j;   // 记录起始位置, 查找相同牌的位置选择显示不同提示
									break;
								}
							}
							...
							{
								// 如果选中的是东南西北, 需要判断是否是4个一样的杠，还是东南西北杠分杠
								// 判断东南西北杠是否存在
								BOOL bGang = FALSE;
								TILE_SET tmpTileSet;
							...
							{
								// 如果选中的是中发白X, 需要判断是否是4个一样的杠，还是中发白X杠
								// 判断中发白X杠是否存在
								BOOL bGang = FALSE;
								TILE_SET tmpTileSet;
								for (j = 0; j < m_stBlockInfo.tilesSet.CurrentLength(); j++)
