...
#define IDM_TAB_SELECT				WM_USER+16					//大厅Tab页选择房间时向房间发送的选择GameRoom消息					
#define IDM_TAB_SELECT_LOOST		WM_USER+17					//大厅Tab页选择房间时，向失去选择的房间发送的GameRoom消息	
#define IDM_OPEN_BANK				WM_USER+19					//打开银行
#define IDM_OPEN_IE					WM_USER+20					//打开IE
#define IDM_MESSAGE_APPEAL          WM_USER+21                  //投诉消息
#define IDM_LOCK_ACCOUNT			WM_USER+22					//锁定机器按钮消息命令

#define IDM_CALL_GM					WM_USER+29					//呼叫GM

#define RIGHT_WND_WIDTH				330							//右侧窗口宽度
...
class EXT_CLASS CNewGameClass/*CNewGameClass*/ : public CWnd
{
	...
	bool m_bIsLBOK;				///< 是否响应鼠标左击事件，用于排队机房间，此值为真，表示响应鼠标点击事件
	bool m_bInit;
	int m_CurWidth;
	//信息变量
protected:
...
public:
	...
	/// 获取当前页面
	int GetCurrentActivePage()const{return m_uActivePage;}
	//内部函数
private:
	//激活窗口
...
public:
	...
	/// 打开或关闭鼠标点击事件
	void EnableLBClick(bool bIsOk){m_bIsLBOK = bIsOk;}


	void ChangeSkin(const CString& skinBcfFileName,const CString& strSkinKey, const CString& skinPath);
public:
	void OnExchangeSkin(const CString& skinBcffile,const CString& strKey,const CString& skinPath);
};
