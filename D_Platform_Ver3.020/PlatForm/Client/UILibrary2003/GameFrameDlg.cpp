...
void CLoveSendClass::OnCancel()
{
	...
	if ((m_bWatchMode==false)&&GetStationParameter() >= 20 && GetStationParameter() < 23)//!CanLeftDesk())
	{
		...
		if (IDCANCEL!=DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION, "提示",true,"你正在游戏中，现在退出将会被扣分，确实要退出吗？"))
			return;
	...
	for(int i=0; i<MAX_PEOPLE; i++)
	{
		...
		if(m_pUserInfo[i])
		{
			if(m_pUserInfo[i]->GameUserInfo.dwUserID == lparam )
			{
				...
				{
					//AFCMessageBox("不能对断线用户使用该道具","提示",AFC_YESNO);
		            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"不能对断线用户使用该道具");
				}
	...
	case IDM_KICK_USER://踢人
		{
			if (m_dwCurrentUserID!=0L)
			{
				...
				if (GetMeUserInfo()->bUserState == USER_WATCH_GAME)
				{
		            ...
		            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"对不起,您正在旁观游戏，不能进行此操作");
					return true;
				}
				...
				if(GetMeUserInfo()->dwUserID == m_dwCurrentUserID)
				{
		            ...
		            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"对不起,无法对自己进行此项操作");
					return true;
				}
				CString stime;
				stime.Format("%d",CTime::GetCurrentTime());        //当前的时间，String格式
				...
				if(GetMeUserInfo()->iVipTime < curtime)
				{
		            ...
		            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"对不起,只有vip才有此项功能");
					return true;
				}
				//处理数据
				UserItemStruct * pUserItem=m_pGameInfo->pIFindUser->FindOnLineUser(m_dwCurrentUserID);
				...
				if(pUserItem->GameUserInfo.iVipTime>curtime)
				{
		            ...
		            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"对不起,对方也是Vip,踢人失败!");
					return TRUE;
				}
...
void CLoveSendClass::UsePropItem(int propIndex)
{
	...
	if(GetMeUserInfo()->bUserState==USER_WATCH_GAME)
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"对不起，旁观时不能使用道具！");
		return;
	}
	//////////////////////////////////////////////////////////////////////////
	...
	{//对自己使用
		...
		if(DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION, "提示",true,stip)==IDYES)
