...
bool CNewGameClass::FixTabPageSize()
{
	...
	for (i=0;i<uPageCount;i++)
	{
		...
		{
			//标签位置
			pPageItem->m_rcTitleRect.SetRect(uBaseXPos+i*uStepX,uBaseYPos+i*uStepY,uBaseXPos+i*uStepX+iWidth,uBaseYPos+i*uStepY+iHeight);//选项卡的大小
...
LRESULT CNewGameClass::OpenBank(WPARAM wparam, LPARAM lparam)
{
	...
	if(m_uActivePage == 0 &&(int)wparam == 1)
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请进入游戏房间再使用呼叫网管功能。");
		return 0;
	}
	if((int)wparam == 0)
	{
