...
bool CFileVersion::Open(LPCTSTR lpszModuleName)
{
    ...
    if ((dwDataSize==0)||(dwDataSize>sizeof(m_bBuffer))) return FALSE;
    if (!::GetFileVersionInfo((LPTSTR)lpszModuleName,dwHandle,dwDataSize,(void * *)m_bBuffer)) return false;
    UINT nQuerySize=0;
    DWORD * pTransTable=NULL;
    if (!::VerQueryValue(m_bBuffer,TEXT("\\VarFileInfo\\Translation"),(void * *)&pTransTable,&nQuerySize)) return false;
