...
bool CLogoUpload::uploadCustomFace(int nBlockIndex)
{
	try
	{
		...
		if(!pParentWnd)
		{
	        ...
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("LogoUploadDlg","Tip","提示"),false,fMsg.GetKeyVal("LogoUploadDlg","SystemError","系统错误"));
			return false;
		}
		//获取当前模块名称
		TCHAR sz[200];
	...
	try
	{
		...
		case ASS_ULS_UPLOADFAILED:
			...
			//AFCMessageBox(fMsg.GetKeyVal("LogoUploadDlg","UploadPicFailed","对不起，自定义头像上传失败，请您稍候再试！"),fMsg.GetKeyVal("LogoUploadDlg","Error","错误"));
	         DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("LogoUploadDlg","Error","错误"),false,fMsg.GetKeyVal("LogoUploadDlg","UploadPicFailed","对不起，自定义头像上传失败，请您稍候再试！"));
			OnBnClickedCancel();
			}
			break;
	...
	try
	{
		...
		if(msg->nBlockIndex==msg->nBlockCount-1)
		{
			...
			if(msg->bNeedCheck)
			{
	            ...
	            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("LogoUploadDlg","Tip","提示"),false,fMsg.GetKeyVal("LogoUploadDlg","UploadPicSuccessWait","自定义头像已上传完毕\n\n在管理员审核通过后重新登录大厅生效！"));
			}
			else
			{
				//AFCMessageBox(fMsg.GetKeyVal("LogoUploadDlg","UploadPicSuccess","自定义头像已上传完毕！"),fMsg.GetKeyVal("LogoUploadDlg","Tip","提示"));//文件已上传完成
	            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,fMsg.GetKeyVal("LogoUploadDlg","Tip","提示"),false,fMsg.GetKeyVal("LogoUploadDlg","UploadPicSuccess","自定义头像已上传完毕！"));
