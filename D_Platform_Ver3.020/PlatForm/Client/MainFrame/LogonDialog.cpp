...
//注册表项定义
#define REG_NAME		TEXT("Software\\BZW系统\\Login\\%s\\UserName")
#define REG_ACC			TEXT("Software\\BZW系统\\Login\\%s\\ACC")
#define REG_MOBILE		TEXT("Software\\BZW系统\\Login\\%s\\MOBILE")
#define REG_ID			TEXT("Software\\BZW系统\\Login\\%s\\ID")
#define REG_SERVER		TEXT("Software\\BZW系统\\Login\\%s\\Logon_Server")
...
void CLogonDialog::Init() 
{
	...
	if((TRUE == g_bRunBySoftware) && (TRUE == g_bAutoConnect))
	{
		...
		m_pPwdEdit->SetText(TEXT("123456"));       // 做个假的，后面更新
...
void CLogonDialog::InitSkin()
{
	...
	//m_pServerCob = static_cast<CComboUI*>(m_pm.FindControl("Cob_Servers"));//Server选择框
	//m_pUserIDCob = static_cast<CComboUI*>(m_pm.FindControl("Cob_ID"));//ID输入框
	//m_pIDEdit = static_cast<CEditUI*>(m_pm.FindControl("Edt_ID"));
	//m_pBkImgae = static_cast<CControlUI*>(m_pm.FindControl("ControlUI1"));
	//m_pPwdEdit = static_cast<CEditUI*>(m_pm.FindControl("Edt_PWD"));//密码输入框
	//m_pThreetName = static_cast<CButtonUI*>(m_pm.FindControl("But_LogonByName"));
	//m_pThreetID = static_cast<CButtonUI*>(m_pm.FindControl("But_LogonByID"));
...
void CLogonDialog::OnOK()
{
	...
	{
		AFCMessageBox("请阅读并勾选同意条款.",MESSAGE_TITLE);
		return;
	}
#ifdef AI_MACHINE
	AILogin();
...
DWORD __stdcall CLogonDialog::ThreadDownConnFile(LPVOID param)
{
	...
	DWORD   dwCurPos = 0;
	CString strFileName;//源文件名
	CString strDestName = "bzconn.temp";//目标文件名
	CString strFullpath;//目标全路径
	CString strPath;	//目标全路径不包括文件名
	TCHAR   szMsg[1000];
	DWORD   err;
	//获取源文件名
	...
	if(!hOpen)
	{
		//显示错误信息
		err = GetLastError();
		wsprintf(szMsg,TEXT("DDownload:InternetOpen错误. %d."),err);
	...
	{
		wsprintf(szMsg,TEXT("DDownload:url为空. %d."),err);
							...
							INTERNET_FLAG_DONT_CACHE|INTERNET_FLAG_PRAGMA_NOCACHE|INTERNET_FLAG_RELOAD,0)))
	{
		...
		{
			wsprintf(szMsg,TEXT("DDownload:InternetOpenUrl 错误.\n网址:%s.\n错误信息:%s."),strDownloadUrl.GetBuffer(),szError);
		}
		else
		{
			wsprintf(szMsg,TEXT("DDownload:InternetOpenUrl 错误.\n网址:%s.\n错误号:%d."),strDownloadUrl.GetBuffer(),err);
		}
		OutputDebugString(szMsg);
		if (pLogonDlg->GetHWND())
		{
	...
	{
		//显示错误信息
		wsprintf(szMsg,TEXT("DDownload:创建文件%s错误. 错误号:%d."),strDestName.GetBuffer(),err);
		OutputDebugString(szMsg);
		if (pLogonDlg->GetHWND())
		{
			CLogonDialog::FillServer(pLogonDlg->m_pServerCob);
	...
	{
		dwByteToRead = 0;
		wsprintf(szMsg,TEXT("DDownload:正在下载%s.... 大小未知."),strFileName.GetBuffer());
		OutputDebugString(szMsg);
	}
	...
	{
		wsprintf(szMsg,TEXT("DDownload:正在下载%s.... 大小%dkbytes."),strFileName.GetBuffer(),dwByteToRead/1024);
	}
	else
	{
		wsprintf(szMsg,TEXT("DDownload:正在下载%s.... 大小%dbytes."),strFileName.GetBuffer(),dwByteToRead/1024);
	}
	OutputDebugString(szMsg);
	...
	do
	{
		if (!::InternetReadFile(hConnect,szTemp,16384,&dwSize))
		{
			...
			if(::InternetGetLastResponseInfo(&err,szError,&len) && lstrcmp(szError,TEXT("")) != 0)
			{
				wsprintf(szMsg,TEXT("DDownload:InternetReadFile 错误.\n 错误信息:%s."),szError);
			}
			else
			{ 
				wsprintf(szMsg,TEXT("DDownload:InternetReadFile 错误.\n 错误号:%d."),err);
