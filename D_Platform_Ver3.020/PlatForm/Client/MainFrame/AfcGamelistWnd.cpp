...
//			{//如果是黑名单中的
//				CBcfFile fMsg(CBcfFile::GetAppPath()+"ClientMessage.bcf");
//				AFCMessageBox(fMsg.GetKeyVal("IMDlg","NoChatWithBadGuyList","对不起，您不能与黑名单中的用户聊天"),
//					fMsg.GetKeyVal("IMDlg","Tip","提示"));
//			}
//			else
//			{
		...
		case ASS_GP_LIST_KIND:		//获取的是游戏类型
			{
				...
				{
					strKey.Format("%02d",i);
					CString strOrderVal =  f.GetKeyVal("Order", strKey, "MDMGameList");//这里默认为面对面游戏，不然配置文件缺失会导致无游戏列表 zht

					if (strOrderVal == "MDMGameList") //面对面的游戏
					{
						m_GameList.HandleKindData((ComKindInfo *)pNetData,uDataSize/sizeof(ComKindInfo));
					}
					else if (strOrderVal == "ExeGameList") //单机游戏
					{
						AddExeGameToList();
					}
					else if (strOrderVal == "WebGameList") //网页游戏
					{
						AddWebGameToList();
					}
					else if (strOrderVal == "3DGameList") //3D游戏
					{
						Add3DGameToList();
					}
				}
	...
	case IDM_DOWN_NEW:				//保存聊天信息
		{
			...
			catch(CException* e)
			{
		        ...
		        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"下载发生了异常！");
...
// 函数名   : CGamePlaceDlg::OnChongZhi
// 说明     : 充值显示
// 返回类型 : void 
// 作者		: Kylin 2009-1-7 18:26:32
...
// 	return;
	...
	{
		//AFCMessageBox("请先退出游戏,再领取!");
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"请先退出游戏,再领取!");
		return;
	}
...
void CGameListWnd::OnMailClick()
{
	if (m_Mails.size() > 0)
	{
	    ...
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,it.szMsg);
	...
	{
		DUIOkeyMsgBox(m_hWnd,false,"您当前没有短消息");
	}
}
