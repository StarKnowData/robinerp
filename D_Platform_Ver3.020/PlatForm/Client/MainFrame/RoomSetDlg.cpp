...
bool CGameRuleSetDlg::SaveSetting()
{
	...
	if (m_bLimitPoint) 
	{
		...
		if (dwHighPoint<=dwLowPoint)
		{
            ...
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("GameRuleSetDlg","ConfigScoreArray","请重新设置积分范围！"));
	...
	if (m_bLimitCut) 
	{
		...
		if ((bCutPercent<0)||(bCutPercent>99))
		{
            ...
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("GameRuleSetDlg","ConfigEscapeRate","请重新设置逃跑率！"));
	...
	if (m_bPass) 
	{
		...
		if (lstrlen(m_szPass)==0)
		{
            ...
            DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("GameRuleSetDlg","ConfigPW","密码不能为空！"));
