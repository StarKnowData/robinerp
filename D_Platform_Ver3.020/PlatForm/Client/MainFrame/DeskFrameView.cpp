...
CBaseDeskView::CBaseDeskView()
{
	...
	m_pFont = new Font(L"宋体", 10, FontStyleRegular);
	m_bCanInput = true;
}
...
bool CBaseDeskView:: InitDesk(CString deskClass,bool deskFlat,UINT uNameID, bool bIsVip)
{
	...
	if (bInit==false)
	{
		...
		////CDesk1::m_PtBasePoint.SetPoint(25,70);							//桌子倍数
		//CDesk1::m_PtBegin.SetPoint(78,65);//正在在游戏的桌面
		//CDesk1::m_IndexRect.SetRect(24,152,50,180);
		//CDesk1::m_PtAgree[0].SetPoint(87,60);
		//CDesk1::m_NameRect[0].SetRect(115,135,200,200);
		//CDesk1::m_ChairRect[0].SetRect(65,67,120,150);//左，上，右，下
...
bool CDeskFrameView::CreateDeskFrame(CWnd * pParentWnd, UINT uWndID)
{
	...
	//end of 百家乐开始按钮add by wlr 20090713
...
//desc:显示百家乐开始按钮 add by wlr 20090713
...
void CDeskFrameView::OnHitQueueBegin()
{
	...
	DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"OnHitQueueBegin");
}
...
{
	m_bIsVip = bIsVip;
	// {{Added by zxd 20091125 从配置文件中读取渐变颜色值
	/// 先从游戏自己的目录下读取
	CString strPrivatePath;
	...
	// Added by zxd 兼容原有.ico模式，先判断是否为ICO模式
	...
	{
		UnInitDeskFrame();
		//AFCMessageBox("游戏初始化失败，请访问www.bzw.cn联系开发者！");
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"游戏初始化失败，请访问www.bzw.cn联系开发者！");
		return false;
	}
	//设置数据
	...
	{
		UnInitDeskFrame();
		//AFCMessageBox("载入桌子图片失败");
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"载入桌子图片失败");
		return false;
	}
	if (m_pImgDesk == NULL) 
	{
	...
	else 
	{
		if (bDeskType!=BJL_DESK_FACE)
		{
	        ...
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"载入锁桌图片失败");
			return false;
		}
	}
...
#endif
		//m_pImgBegin = new Image(pBeginImage, FALSE);
		//m_BeginImage.Attach(pBeginImage,CGameImageLink::m_bAutoLock);//pBeginImage是正在在游戏的桌面m_PtBegin透明图
	}
	else 
	{
		if (bDeskType!=BJL_DESK_FACE)
		{
			UnInitDeskFrame();
			//AFCMessageBox("载入开始游戏桌图失败");
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"载入开始游戏桌图失败");
			return false;
		}
	}
	...
	else 
	{
		...
		{
			UnInitDeskFrame();
			//AFCMessageBox("载入同意图失败");
	        DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"载入同意图失败");
			return false;
		}
	}
								   ...
								   BYTE bDeskType,int uComType,GameInfoStruct *GameInfo,CString deskClass,bool deskFlat,COLORREF iclrSitDeskFrameLeftTop,COLORREF iclrSitDeskFrameRightBottom)
{
	...
	{
		// {{Modified by zxd 兼容原有.ico模式，载入桌子图片
		if (bIsIco)
		{
			switch (bEveryCount)
			{
...
{
     // PengJiLin, 2011-6-23, 激活消息//zht 2012-4-23 GDI+修改卡屏问题
   /* if(FALSE == m_bActive) 
    {
        CPaintDC dc(this);
        return;
...
void CDeskFrameView::OnPaintPng()
{
		...
		)
	{
		...
		//SolidBrush solidBrush(Color(255, 255, 255, 255));  //   第一种颜色
   ...
   //     0, 0,backgroundtop.GetWidth(),backgroundtop.GetHeight(),UnitPixel,&imAtt);
	...
	if ((m_byDeskType==BJL_DESK_FACE) || (1 == m_bDeskCount && m_deskClass != "queue" ))//)
	{
		...
		if (pDesk)
		{
			...
			{					
				showBJLBeginOrPlayingBtn(m_bShowPlaying);//百家乐 add by wlr 20090717
			}				
		}
	}//end of 百家乐 add by wlr 20090713
	...
	///////////////////////////rqf zht 2012-4-23 GDI+部分电脑修改卡屏问题
	Color c;
	...
	if (pGameInfo->dwRoomRule & GRR_CONTEST)
	{
		...
		{
			szInfo.Format("第 %d 局", pGameInfo->pMeUserInfo->GameUserInfo.iContestCount);
		}
		else
		{
			szInfo = "第 1 局";
		...
		if (pGameInfo->pMeUserInfo != NULL)
		{
			...
			{
				szInfo.Format("当前您的排名：%d",pGameInfo->pMeUserInfo->GameUserInfo.iRankNum);
			}
			else
			{
				szInfo.Format("当前您的排名：");
			}
...
void CDeskFrameView::OnPaintBmp()
{
	...
	if (m_pDeskArray!=NULL)
	{
		...
		//BufferDC.SetBkMode(TRANSPARENT);//透明的, 显然的, 明晰的背景颜色
		//BufferDC.SelectObject(&CGameImageLink::m_HFont);//显示字体
		//BufferDC.SetTextColor(CGameImageLink::m_ColorResource.m_crDeskTxColor);//选择的文本颜色
		//BufferBmp.CreateCompatibleBitmap(&dc,m_iPicWidth,m_iPicHight);//创建兼容位图
		//CBitmap * pOldBmp=BufferDC.SelectObject(&BufferBmp);


		CGameImageHelper DeskImageHandle(&m_DeskImage);//桌子图片
		CGameImageHelper SetBasePointImageHandle(&m_TableBasePoint);//桌子倍数图片
		CGameImageHelper LockImageHandle(&m_LockImage);//锁图片
		CGameImageHelper ArgeeImageHandle(&m_AgreeImage);//同意图片
		...
		//百家乐 add by wlr 20090713
		CBaseDeskView * pDesk=NULL;
		if (1 == m_bDeskCount)//只有一张桌子，居中显示 add by ramon
		{
			//dc.FillSolidRect(&ClientRect,crBackColor);
			pDesk=GetDeskArrayPtr(0);
			if (pDesk)
...
void CDeskFrameView::OnSize(UINT nType, int cx, int cy)
{
	...
	if(m_iPicWidth==0 || m_iPicHight==0)
	{
	    ...
	    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,"System Failed ");
		return;
	}
	if (m_bDeskCount!=0)
	{
...
void CDeskFrameView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pGameRoom != NULL)
	{
		//if (pGameRoom->IsQueueGameRoom())//为排队机添加
		//{
		//	return;
		//}
	}
	...
	if ((m_bRightMouseDown==false)&&(m_bLeftMouseDown==false))
	{
		...
		//隐藏右键弹出的用户信息 modify by lxl 2010-11-8
