...
void CBankDlg::OnBnClickedCheckout()
{
	...
	if(m_BankMoney<=0)
	{
		...
		if(IDYES == DUIMessageBox(m_hWnd,MB_YESNO|MB_ICONINFORMATION,fMsg.GetKeyVal("BankDlg","TipBank","银行"),true,fMsg.GetKeyVal("BankDlg","NoCoinCharge","您已无金币，请充值!")))
		{
			CString s=CBcfFile::GetAppPath ();/////本地路径
...
//	if(In.DoModal()==IDOK)
	...
	CBcfFile fMsg(CBcfFile::GetAppPath()+"ClientMessage.bcf");
	{
			...
			for (BYTE i=0;i<MAX_GAME_ROOM;i++)
			{
				if (pRoomInfo[i].bAccess)//判断是否在游戏中
				{
					...
					if(!p->CanCloseNow())
					{
		                ...
		                DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("BankDlg","StartingNotSave","游戏已经开始无法存入金币!"));
						return ;
					}
				}
			}
		...
		if (m_pMeUserInfo->GameUserInfo.bUserState == USER_PLAY_GAME)
		{
		    ...
		    DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("BankDlg","PlayingNotSave","您当前正在游戏中，无法进行存入金币操作!"));
			return ;
		}
...
bool CBankDlg::GetRoomType(int uComType)
{
	if(uComType<1||uComType>3)
	{
		...
		DUIMessageBox(m_hWnd,MB_ICONINFORMATION|MB_OK,"系统提示",false,fMsg.GetKeyVal("BankDlg","RoomTypeError","房间类型错误!"));
		return false;
	}
	m_uComType=uComType;
	return true;
