		...
		for(int i=0; i<pWnd->m_UserList.GetCount(); i++)
		{
			...
			if(p->UserID ==msg->dwRequestedUserID)
			{
				...
				if (p->iAddFriendType % 65535 == 0)
					//pWnd->m_TCPSocket.SendData(p->iSocketIndex,msg,msg->cbSize,MDM_GP_IM,ASS_IMS_ADDREQUEST,0,0);
					pWnd->SendDataByUser(msg->dwRequestedUserID,msg,msg->cbSize,MDM_GP_IM,ASS_IMS_ADDREQUEST,0,0);
				else
					//pWnd->m_TCPSocket.SendData(uIndex,msg,msg->cbSize,MDM_GP_IM,ASS_IMC_ADDREQUEST,1,0);
...
void CIMService::OnNetAddResponse(void * pData, UINT uSize, UINT uIndex, DWORD dwHandleID)
{
	...
	//	if(p == NULL)
	//		continue;
	//	if(p->UserID ==msg->dwUserID)
	//	{
	//		pWnd->m_TCPSocket.SendData(p->iSocketIndex,msg,sizeof(MSG_IM_S_ADDRESPONSE),MDM_GP_IM,ASS_IMS_ADDREQUESTRESPONSE,0,0);
	//		
	//		if (++ iCount == 2)  //yjj 090323
	//		{
	//			break;
	//		}
	//		
	//	}
	//	else if (p->UserID == tempmsg.dwUserID)
	//	{

	//		pWnd->m_TCPSocket.SendData(p->iSocketIndex,&tempmsg,sizeof(MSG_IM_S_ADDRESPONSE),MDM_GP_IM,ASS_IMS_ADDREQUESTRESPONSE,0,0);
