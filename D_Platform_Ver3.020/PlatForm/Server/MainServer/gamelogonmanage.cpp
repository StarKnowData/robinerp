...
{
	if(pNetHead->bMainID == MDM_GR_MONEY)  // 转账 add by xqm 2010-11-9
	{
	...
	if (pNetHead->bMainID==MDM_GP_LOGON)		//用户登陆，，网络头的主标识为MDM_GP_LOGON
	{
        ...
        else if(ASS_GP_LOGON_BY_SOFTWARE == pNetHead->bAssistantID) // PengJiLin, 2010-6-2, 第三方启动通知。(鸥朴)
        {
            m_bRunBySoftware = TRUE;
            return true;
        }
	}
	else if(pNetHead->bMainID == MDM_GP_LOCK_ACCOUNT )	//锁定机器 zxj 2009-11-13
	{
	...
	else if (pNetHead->bMainID==MDM_GP_LIST)	//构件列表
	{
		...
		case ASS_GP_ROOM_PASSWORD:	// 验证某房间的密码 add by lxl 2010-10-08
			{
				//效验数据
	...
	//added by JianGuankun 2011.11.08修改用户信息
...
UINT CServiceDataBaseHandle::HandleDataBase(DataBaseLineHead * pSourceData)
{
	...
	{
		///{{ Added by zxd 20100802 用于GM模块处理过程
	...
	case DTK_GP_CHECK_ROOM_PASSWD:  //试图进入加密房间 add by lxl 2010-10-08
		{
	...
	case DTK_GP_PROP_BUY_VIP:
		OnPropBuyVIP(pSourceData);    //道具界面购买VIP add by wyl  11-5-21
	...
	case DTK_GP_LOCK_UNLOCK_ACCOUNT:
		OnUserLockAccount(pSourceData);	//锁定机器 zxj 2009-11-13
		break;
	...
	case DTK_GP_CHECK_SELLGAMELIST:				//wyl 2011-5-16  判断是否发送消息列表
		{
	...
	///{{ Added by JianGuankun 2011.12.20 用于银行处理模块
...
void CServiceDataBaseHandle::OnPropGetList(DataBaseLineHead * pSourceData)
{
	...
	HANDLE hDatabase=m_pDataBaseManage->hDBLogon;
	HANDLE hRecord=sqlSPSetNameEx(hDatabase,"SP_NewPropGetByUser"/*"SP_PropGetByUser"*/); // PengJiLin, 2010-10-12, 新的道具系统功能
	if(hRecord<(HANDLE)0x1000)
		return;
	try
	{
		...
		while(!sqlEndOfFile(hDatabase,hRecord))
		{
			...
			//if(propOGet.msgPropGet.nHoldCount>0)  // PengJiLin, 2010-10-12, 注释，使用新的道具系统功能
	...
	case DTK_GP_LOGON_BY_MOBILE:	//通过手机登陆
		{
			...
			else
			{
                ...
                // PengJiLin, 2010-6-3, 如果是第三方启动，部分错误原因需要提交
                // 第三方认证处理。(鸥朴)
...
UINT CServiceDataBaseHandle::OnPlaceLogon(DataBaseLineHead * pSourceData, UINT uLogonType)
{
	...
	try
	{
        ...
        // PengJiLin, 2010-6-3, 如果是第三方启动，当登录失败时，需要转第三方认证
		...
		//{由于锁定机器的原因，所以增加几个报错的功能 zxj 2009-11-12
		if(ret==6)
		{
		...
		if(ret==0)
		{
			...
			sqlGetValue(hDatabase,hRecord,TEXT("LockMathine"), DT_LogonResult.LogonResult.iLockMathine);	//返回锁定状态 zxj 2009-11-13
			sqlGetValue(hDatabase,hRecord,TEXT("BindMobile"), DT_LogonResult.LogonResult.iBindMobile);	//返回手机的绑定状态 jianguankun 2012-10-10
...
void CGameLogonManage::GetINIFile()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		m_strSmsContent[i] = cfgGetValue(cfgHandle,"Plugin",szKey,"您好%s，短信验证码：%s。");
	}
	...
	//s=f.GetKeyVal ("www.BZW.cn","BZW","");//服务器参数
	//m_uMaxPeople=f.GetKeyVal (s,"uMaxPeople",300);//支持最大人数

	//m_nPort=f.GetKeyVal (s,"m_iMainserverPort",3015);//登陆服务器端口
